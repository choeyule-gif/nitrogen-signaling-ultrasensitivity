...
## Generalization and expanded calibration / v1.4.0

`validation/integrated_checks_v1.4.0.json` records all 13 current stages passing on the designated Linux computation server. The default command independently recounts 840,000 stored evaluation decisions and replays selected complete raw trials, with 50 alternative nonlinear refits. It freshly evaluates the graph/cofactor algorithm, AceK conserved steady states and integrations, source-summary binding models and withheld ATPase halfpoints. It is a verification run, not a second clean regeneration of all Monte Carlo cells.

The expanded calibration itself completed 3,000 trials in each of 27 null cells and 2,000 independent trials in each of 210 evaluation cells, per strategy. Each trial has eight nonlinear fits. Thus there are 8,016,000 fits across shared and condition-specific signal models; their underlying raw observations are paired by seed. Null calibration and evaluation use separate seed phases. Grid-conditional calibration, continuous-mixture stress tests, concentration and signal errors, optimizer failures and active-bound events are documented in `extras/bootstrap_design/README.md`, with all cell records and environment metadata. Use new output folders to regenerate the large study from scratch; the generator intentionally resumes existing compatible cells.

The second biological system uses the published AceK–IDH topology with singly and doubly bound dimers. An independent exact cofactor implementation corroborates the separate hand elimination. Four additional graphs, a rank-one control and an infeasible-observation control check the general algorithm. Six published biochemical summaries calibrate initial-rate constructions; three withheld ATPase halfpoints challenge additional biological implications. They are not replicate-level experimental validation of the entire native cycle. The independently transcribed cellular activity table has different units and assay conditions and is not used to manufacture a matched count gate.

All current figure sources and document mappings are in `manuscript/README.md`. The previous paragraph describing six supplements belongs to earlier releases; the current version has three appendices and nine supporting figures. The package metadata and S1 Data README use the same current title and v1.4.0 source identifier. A version DOI is reported only after the corresponding published archive is verified; the concept DOI is explicitly version-independent.

For publication packaging, install the optional `publication` dependencies and provide pdfLaTeX and Poppler. `scripts/build_submission.py` assembles already compiled documents and reviewed figures; it does not rerun scientific simulations. The public source release contains the manuscript sources, while S1 Data focuses on analysis code, data and verification.
