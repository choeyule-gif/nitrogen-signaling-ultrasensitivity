...
[fig, ax] = newPanel();
plot(ax,piiLevels,coefficients.calculated,'-o','Color',colors(1,:),'MarkerFaceColor',colors(1,:),'LineWidth',2,'DisplayName','calculated');
plot(ax,piiLevels,coefficients.reported,'-s','Color',colors(2,:),'MarkerFaceColor',colors(2,:),'LineWidth',2,'DisplayName','reported');
ax.XScale='log';
xlim(ax,[0.2,100]);
ylim(ax, [0, 8]);
xticks(ax, piiLevels);
