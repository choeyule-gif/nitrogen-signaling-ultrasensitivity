...
plot(ax, points.glutamine_mM, points.UMP_per_trimer, 'ko', 'MarkerFaceColor', [0.15,0.15,0.15], 'MarkerSize', 5, ...
...
for k = 1:numel(exponents)
    plot(ax, residuals.glutamine_mM, residuals.(sprintf('r_h%d', exponents(k))), '-o', ...
        'Color', exponentColors(k, :), 'MarkerFaceColor', exponentColors(k,:), 'MarkerSize', 5, ...
        'LineWidth', 2, 'DisplayName', sprintf('h = %d', exponents(k)));
end
...
[fig, ax] = newPanel();
sample=sort(draws{:,1});
stairs(ax,sample,(1:numel(sample))'/numel(sample),'Color',blue,'LineWidth',2);
