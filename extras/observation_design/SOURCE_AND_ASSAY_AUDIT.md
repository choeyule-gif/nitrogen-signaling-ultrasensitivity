...
The GlnD RMSE comparison changes multiple constraints. The earlier 0.540 fit fixes effective C=100, q1=q0 and the transported UR half-range 0.080 mM, in addition to the two endpoint specificity ratios. Its state capacities are (1,1,d2), not a common constant. The 0.0409 fit retains the two endpoint ratios and activity ordering but releases the other restrictions; its UR half-range is approximately 0.256624 mM. No single-relaxation attribution follows. The binding benchmark K_G range [7.8,15.6] mM is source-anchored; the earlier [7.8,31.2] mM range is a factor-two analytic sensitivity scenario, not an uncertainty region covered by the new Monte Carlo results.
