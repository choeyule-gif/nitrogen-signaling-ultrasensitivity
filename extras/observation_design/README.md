...
The v1.3.2 comparison audit reconstructs the earlier fixed-C model and computes both fitted UR half-ranges independently. Both endpoint ratios are retained, but fixed C, the intermediate-state restriction and the transported 0.080 mM half-range are jointly relaxed in the newer fit. The fitted half-range is 0.256624 mM. The improvement cannot be attributed to variable state capacity alone.
