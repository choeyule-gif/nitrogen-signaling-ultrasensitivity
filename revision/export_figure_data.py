...
csvw("Fig3D_swing_profile.csv", dict(swing=ww, loss=loss), "Fig 3D: profile likelihood loss N log(SSE_W/SSE_free) against the regulatory swing W = f_max/w_0; threshold 3.841; lower endpoint %.2f" % wlo)
csvw("Fig3D_markers.csv", dict(quantity=["threshold", "lower95", "point_estimate"], value=[3.841458820694, wlo, swing]), "Fig 3D reference values")
...
csvw("Fig2B_eta_max.csv", dict(affinity_ratio=rat, eta_max=2*np.sqrt(rat)/(1+np.sqrt(rat))), "Fig 2B: maximum switch elasticity of eq (6) against A_UT/A_UR")
csvw("Fig2B_markers.csv", dict(ion=["Mg", "Mn"], affinity_ratio=[80/70, 700/150], eta_max=[2*np.sqrt(80/70)/(1+np.sqrt(80/70)), 2*np.sqrt(700/150)/(1+np.sqrt(700/150))]), "Fig 2B markers from the reported kinetic constants")
xx = np.logspace(-2, 1.3, 400)
csvw("Fig2C_local_slope.csv", dict(glutamine_mM=xx, nH_loc=nlocal(xx, p)), "Fig 2C: local logit slope of the fitted physical occupancy; fitted exponent %.4f; value at S = %.4f" % (p[3], out['local_slope_at_S']))
csvw("Fig2C_markers.csv", dict(quantity=["S_mM", "nH_loc_at_S", "nH_fit"], value=[S, out['local_slope_at_S'], p[3]]), "Fig 2C markers")
...
csvw("Fig3C_markers.csv", dict(quantity=["h_fit", "p025", "p975"], value=[p[3], q_h[0], q_h[2]]), "Fig 3C markers")
...
csvw("Fig4A_composites.csv", cols, "Fig 4A: calibrated composite responses normalised between their plateaus; midpoints at the reported cascade half-points")
csvw("Fig4A_midpoints.csv", dict(PII_uM=pii, midpoint_mM=mid), "Fig 4A midpoints")
csvw("Fig4B_coefficients.csv", dict(PII_uM=pii, calculated=pred, reported=observed), "Fig 4B: calculated range coefficients (linear reading of the reverse input) and reported cascade coefficients")
csvw("Fig4C_ratios.csv", dict(PII_uM=pii, ratio=res, p025=q_res[0], p975=q_res[2]), "Fig 4C: reported / calculated with central 95%% ranges over the 2,000 upstream coordinate perturbations")
xx = np.logspace(-2, 2, 300)
csvw("Fig5A_binding.csv", dict(scaled_glutamine=xx, one_ligand=xx/(1+xx), two_ligands=2*xx**2/(1+xx**2)), "Fig 5A: ligands bound per enzyme for a one-site law and a concerted two-site law")
