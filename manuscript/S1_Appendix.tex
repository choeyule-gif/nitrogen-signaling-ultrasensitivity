...
For f(l)=C l(1+$A_{UT}$ l)/(1+$A_{UR}$ l), direct logarithmic differentiation gives main-text Eq. (6). If $A_{UT}$>$A_{UR}$, differentiating once more locates the maximum at l=$1/\sqrt{A_{UT}A_{UR}}$, where
...
If the affinities are equal, $\eta$=1. If $A_{UT}$<$A_{UR}$, the stationary point is a minimum and the supremum is one at the limiting inputs. The $\mathrm{Mg}^{2+}$ and $\mathrm{Mn}^{2+}$ values in Fig. 2B use inverse reported kinetic concentrations as model affinities: 80/70 and 700/150 for the two ratios. This is an explicit model identification, not an independent equilibrium-binding measurement.
...
At the fitted midpoint, this gives 1.725 for the reconstructed curve, whereas its range coefficient is 2.018. Figure 2C displays this distinction directly. The fitted exponent coincides with a ligand count only in a separately specified regulatory limit.
...
This is a counterexample, not evidence for hidden GlnD/GlnE conformations or identical transients. Figure 11 distinguishes only its specified classes and nuisance ranges. A larger state space requires another discriminating observable or independent restrictions on catalytic states.
