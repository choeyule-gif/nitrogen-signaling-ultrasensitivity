...
An additional paired GlnB/GlnK design uses a binding-disabled response-curve calibration, allowing conditional sequestration intervals without identifying the GlnD switch. The construction permits interacting free ladders, bounded reporter partition and imperfect bound-target selectivity when the required calibration and uncertainty bounds hold. Independent published peptide measurements establish available observables, but are not presented as measured sequestration.

The five-figure manuscript combines exact derivations, published measurements, and independently checked numerical reaction models. No new experimental measurements were generated. Digitized coordinates, prediction grids, parameter provenance, and verification outputs are supplied as S1 Data; the existing analysis is publicly available through GitHub and its versioned archive. The generalized calibration extension is supplied with the current S1 Data.
